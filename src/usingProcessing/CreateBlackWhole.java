...
public class CreateBlackWhole extends PApplet {
	int num = 1000;
	TestPVector[] mys = new TestPVector[num];

	public static void main(String[] args) {
		// TODO 自動生成されたメソッド・スタブ
		PApplet.main(new String[] {CreateBlackWhole.class.getName()});
	}
...
//			mys[i].location.set(width/2.0f, height/2.0f);
			mys[i].location.set(random(width), random(height));
//			mys[i].acceleration.set(random(-10, 10), random(-10, 10));
			mys[i].gravity.set(0.0f,0.0f);
...
*/
	...
	class TestPVector{
		...
		public void attract(PVector center, float _mass, float min, float max) {
			...
			float strangth = 0.1f * (mass * _mass) / (distance * distance);
